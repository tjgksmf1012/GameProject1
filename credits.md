...
캐릭터 4단계 연기 시트 10종, 타이틀 점포 배경, CCTV 4채널 아틀라스는 OpenAI의 내장
이미지 생성 도구로 2026-08-19~2026-08-21에 이 프로젝트를 위해 제작한 사전 생성
래스터 원화다. 실존 브랜드,
상표, 저작권 캐릭터, 특정 작가 이름을 프롬프트에 사용하지 않았다.

파일별 용도와 최종 프롬프트는 `docs/art/GENERATED-ASSETS.md`와
`docs/art/CHARACTER-PERFORMANCE-ASSETS.md`에 기록한다. 이 원화는
게임 PCK에 포함되므로 Steam 출시 시 Pre-Generated AI 콘텐츠 공시가 필요하다.
